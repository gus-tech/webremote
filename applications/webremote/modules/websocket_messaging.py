...
class PostHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if 'message' in self.request.arguments:
            ...
            print('{}:MESSAGE to {}:{}'.format(time.time(), group, message))
            if hmac_key:
...
class DistributeHandler(tornado.websocket.WebSocketHandler):
    ...
    def open(self, params):
        ...
        print('{}:CONNECT to {}'.format(time.time(), self.group))
    ...
    def on_message(self, message):
        ...
        if   '1' in obj: return bt.send_code(obj['1'])
        elif '2' in obj: return ir.send_code(obj['2'])
        elif '3' in obj:
            data = obj['3']
            code = ir.learn_code()
            data['code'] = json.dumps(code)
            data = wr.add_remote_button(db, data)
            for client in listeners.get(self.group, []): client.write_message(json.dumps(data))
        else:
            try:
                key = obj.keys()[0]
                print(key)
                ...
                for client in listeners.get(self.group, []): client.write_message(temp_dict)
            except Exception as error:
                print("Error on message:", error)
    ...
    def on_close(self):
        ...
        #for client in listeners.get(self.group, []):
        #    client.write_message('-' + self.name)
        print('{}:DISCONNECT from {}'.format(time.time(), self.group))
